...
% trial and user's defined group(s) of animals.
    ...
    % demo equalize groups
    if length(user_groups) > 1
        if user_groups(1) == 1 && user_groups(2) == 3
            %4966
            animals_trajectories_map{1}(:,10) = [];
        elseif user_groups(1) == 2 && user_groups(2) == 3
            %5772
            animals_trajectories_map{1}(:,3) = [];
        end
    end      
